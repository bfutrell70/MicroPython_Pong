...
# X, Y, width, height
#paddle = Paddle(0, int((64 - paddleHeight) / 2), paddleWidth, paddleHeight)
paddle = Paddle(0, int((64 - playerPaddleHeight) / 2), paddleWidth, playerPaddleHeight, "player")
computerPaddle = Paddle(127, int((64 - paddleHeight) / 2), paddleWidth, paddleHeight, "computer")
...
# next two statements cause an error:
# "AttributeError: 'ADC' object has no attribute 'Width'"
# not sure if this is specific to ESP32 or not
...
while True:
    if (game <= maxGames):
        ...
        # update the xPos/YPos coordinates
        # *** for pong we don't need to move left/right...
        # UpdateX()
        paddle.MovePaddle(joystick.CheckY(y), 1, 62)
        
        # move the computer paddle
        # 3/14/2022 - NOTE: this code essentially makes the computer's paddle
        #   stay in line with the ball.
        #   Need to update the logic for the computer paddle so that it doesn't
        #   move toward the ball until it is a certain distance from the paddle.
        #if (ball.y > computerPaddle.y):
        #    computerPaddle.MovePaddle(1, 1, 62)
        #if (ball.y < computerPaddle.y):
        #    computerPaddle.MovePaddle(-1, 1, 62)
        # Updated rule - the computer only moves to the ball once it is a 
        #   specified distance from the paddle.
        if (computerPaddle.UpperLeftX() - ball.x <= computerReactDistance):
            ...
            # of the time.
            if (random.randint(1, 100) <= computerReactionChance):
                if (ball.y > computerPaddle.y):
                    computerPaddle.MovePaddle(1, 1, 62)
                if (ball.y < computerPaddle.y):
                    computerPaddle.MovePaddle(-1, 1, 62)
        ...
        # minX, maxX, minY, maxY
        ballValue = ball.MoveBall(1, 126, 1, 62)
        ...
        if (ballValue == -1):
            print('player missed ball!')
            DrawMissedBall()
            Reset()
        
        # check if the ball passed the computer's paddle
        if (ballValue == -2):
            print('computer missed ball!')
